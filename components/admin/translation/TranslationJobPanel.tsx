...
type Provider = 'azure' | 'openai_paid' | 'gemini';
type TranslationItem = { key: string; text: string };
type RunResult = TranslationItem & { translatedText: string; memoryHit: boolean };
type Quota = { used: number; reserved?: number; limit: number; remaining: number; status: string };
...
export default function TranslationJobPanel({ trainerId }: { trainerId: number }) {
  ...
  const setProviderSafely = (next: Provider) => { setProvider(next); setPaidConsent(false); setPreview(null); setJob(null); setDrafts({}); setIdempotencyKey(''); setLimitDraft(''); setLimitMessage(''); setError(''); setPhase('idle'); };

  const saveMonthlyLimit = async () => {
    const hardLimitCharacters = Number(limitDraft);
    if (!Number.isSafeInteger(hardLimitCharacters) || hardLimitCharacters < 0) { setLimitMessage('Enter a whole number of characters.'); return; }
    setLimitSaving(true); setLimitMessage('');
    try {
      await readResponse(await fetch('/api/admin/translations/usage', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ provider, hardLimitCharacters }) }));
      setLimitMessage('Monthly limit saved. Refresh the preview before running a translation.');
      setPreview(null); setIdempotencyKey('');
    } catch (reason) { setLimitMessage(reason instanceof Error ? reason.message : 'Could not save the monthly limit.'); }
  ...
  return <section className="rounded-2xl border border-cyan-500/20 bg-cyan-950/10 p-5 space-y-5" aria-labelledby="translation-job-title">
    ...
    {(provider === 'gemini' || provider === 'openai_paid') && <div className="rounded-lg border border-amber-500/30 bg-amber-950/20 p-3 text-xs text-amber-100"><p>Set a monthly character limit before automated Gemini or OpenAI use. New months start at 0 until a limit is set.</p><div className="mt-3 flex flex-wrap items-center gap-2"><input inputMode="numeric" min="0" step="1" value={limitDraft} onChange={(event) => setLimitDraft(event.target.value)} placeholder="Monthly character limit" disabled={busy || limitSaving} className="w-52 rounded-lg border border-amber-500/30 bg-slate-950 p-2 text-xs text-white" /><button type="button" onClick={saveMonthlyLimit} disabled={busy || limitSaving || limitDraft.trim() === ''} className="rounded-lg border border-amber-400/50 px-3 py-2 text-xs font-bold text-amber-100 disabled:opacity-50">{limitSaving ? 'Saving…' : 'Save monthly limit'}</button>{limitMessage && <span role="status">{limitMessage}</span>}</div></div>}
    ...
    <div className="flex flex-wrap items-center gap-3"><button type="button" onClick={requestPreview} disabled={busy || !source?.items.length} className="rounded-lg border border-cyan-500/40 px-4 py-2 text-xs font-bold text-cyan-300 disabled:opacity-50">{phase === 'previewing' ? 'Previewing…' : 'Preview estimate'}</button><button type="button" onClick={runTranslation} disabled={busy || !preview || preview.quota.status !== 'available' || (provider === 'openai_paid' && !paidConsent)} className="rounded-lg bg-cyan-500 px-4 py-2 text-xs font-bold text-slate-950 disabled:opacity-50">{phase === 'running' ? 'Running…' : 'Run translation'}</button><span className="text-xs text-slate-400">Status: <strong className={['error','conflict','quota_blocked','unavailable'].includes(displayedPhase) ? 'text-rose-400' : 'text-cyan-300'}>{displayedPhase}</strong></span>{busy && <Loader2 className="w-4 h-4 animate-spin text-cyan-400" />}</div>
