...
const copy = {
  ko: { title: '자주 묻는 질문', sub: '파일 처리, 백신 경고 및 안전 이용 안내', guide: '전체 안전 가이드 보기', items: [
    ...
    [Lock, '선택한 트레이너 파일이 서버에 저장되나요?', '변환 대상 파일은 브라우저에서 로컬로 처리되며 LocalPatcher 자체 데이터베이스에 업로드하거나 저장하지 않습니다. 다만 GA4와 AdSense 같은 제3자 서비스가 접속·기기·이용 정보를 처리할 수 있습니다. 자세한 내용은 개인정보처리방침을 확인하세요.'],
  ]},
  en: { title: 'Frequently Asked Questions', sub: 'File processing, antivirus alerts, and safer use', guide: 'Read the full safety guide', items: [
    ...
    [Lock, 'Is my selected trainer file stored on a server?', 'The selected file is processed locally in your browser and is not uploaded to or stored in LocalPatcher’s own database. Third-party services such as GA4 and AdSense may process access, device, and usage information. See the Privacy Policy for details.'],
  ]},
  ja: { title: 'よくある質問', sub: 'ファイル処理、ウイルス対策の警告、安全利用について', guide: '安全ガイドをすべて確認', items: [
    ...
    [Lock, '選択したファイルはサーバーに保存されますか？', '対象ファイルはブラウザ内でローカル処理され、LocalPatcher独自のデータベースへアップロード・保存されません。ただしGA4やAdSenseなどの第三者サービスがアクセス・端末・利用情報を処理する場合があります。詳細はプライバシーポリシーをご覧ください。'],
  ]},
} as const;
