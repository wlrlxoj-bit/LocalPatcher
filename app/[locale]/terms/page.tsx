...
const content: Record<Locale, Content> = {
  ko: {
    ...
    privacy: '개인정보 및 분석·광고 서비스에 관한 자세한 사항은 개인정보처리방침을 확인해 주세요.',
    sections: [
      ...
      { icon: ShieldCheck, title: '4. 광고, 분석 및 이용 이벤트', text: '무료 운영을 유지하기 위해 Google AdSense 등 광고 서비스와 Google Analytics 4 등 분석 도구를 사용할 수 있습니다. 이름·이메일 등 직접 식별정보를 이벤트 매개변수로 붙이지 않은 페이지 방문, 다운로드 시작, 광고 페이지 열림, 팝업 차단 또는 광고 차단 감지 등의 이용 이벤트가 측정될 수 있으며, 제3자 서비스가 쿠키·기기 정보·접속 정보를 처리할 수 있습니다.' },
      ...
      { icon: FileText, title: '8. FLiNG, 외부 링크 및 광고 페이지', text: '서비스는 편의를 위해 FLiNG 공식 페이지를 포함한 외부 사이트, 다운로드 출처 또는 광고 페이지로 연결할 수 있습니다. 외부 사이트는 각 운영자의 정책과 책임 아래 제공되며 LocalPatcher는 그 내용, 파일, 보안, 가용성 또는 거래를 통제하거나 보증하지 않습니다. 링크를 열기 전에 주소와 파일 출처를 확인하세요.' },
  ...
  en: {
    ...
    privacy: 'For details about privacy and analytics or advertising services, please review the Privacy Policy.',
    sections: [
      ...
      { icon: ShieldCheck, title: '4. Advertising, analytics, and usage events', text: 'To support free operation, the Service may use advertising services such as Google AdSense and analytics tools such as Google Analytics 4. Usage events such as page visits, download starts, ad-page openings, blocked popups, or ad-block detection may be measured without attaching direct identifiers such as names or email addresses as event parameters, and third-party services may process cookies, device information, and connection data.' },
      ...
      { icon: FileText, title: '8. FLiNG, external links, and ad pages', text: 'For convenience, the Service may link to external websites, download sources, or advertising pages, including official FLiNG pages. Those services operate under their own policies and responsibility. LocalPatcher does not control or guarantee their content, files, security, availability, or transactions. Verify the address and source before opening or downloading.' },
  ...
  ja: {
    ...
    privacy: 'プライバシーおよび分析・広告サービスの詳細については、プライバシーポリシーをご確認ください。',
    sections: [
      ...
      { icon: ShieldCheck, title: '4. 広告、分析および利用イベント', text: '無料運営を維持するため、Google AdSense等の広告サービスおよびGoogle Analytics 4等の分析ツールを使用する場合があります。氏名やメールアドレス等の直接識別情報をイベントパラメータとして付加せず、ページ閲覧、ダウンロード開始、広告ページの表示、ポップアップのブロック、広告ブロックの検知等の利用イベントが計測される場合があります。また、第三者サービスがCookie、端末情報、接続情報を処理する場合があります。' },
      ...
      { icon: FileText, title: '8. FLiNG、外部リンクおよび広告ページ', text: '利便性のため、FLiNG公式ページを含む外部サイト、ダウンロード元または広告ページへのリンクを提供する場合があります。外部サイトは各運営者の方針と責任で提供され、LocalPatcherはその内容、ファイル、安全性、可用性または取引を管理・保証しません。アクセスやダウンロードの前にURLと提供元を確認してください。' },
