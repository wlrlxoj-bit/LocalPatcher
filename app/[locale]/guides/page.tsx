...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = (content as Record<string, any>)[currentLocale] || content.en;
...
export default async function GuidesPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <main className="max-w-4xl mx-auto px-5 py-10 md:py-14">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {t.sections.map(([title, text]: [string, string]) => <section key={title} className="p-6 rounded-2xl border border-slate-800 bg-slate-900/20"><CheckCircle2 className="w-5 h-5 text-cyan-400 mb-3" /><h2 className="font-bold text-white">{title}</h2><p className="mt-2 text-sm text-slate-400 leading-relaxed">{text}</p></section>)}
    </div>
