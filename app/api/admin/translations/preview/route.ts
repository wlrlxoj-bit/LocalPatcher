...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isTranslationProvider(provider) || !Number.isInteger(body.trainerId) || !isTargetLanguage(body.targetLanguage)) return NextResponse.json({ error: 'invalid_request' }, { status: 400 });
