...
import { isTargetLanguage, isTranslationProvider, type TranslationProvider } from '@/lib/server/translation/types';
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isTranslationProvider(body.provider) || !Number.isInteger(body.trainerId) || !isTargetLanguage(body.targetLanguage) || !body.previewHash || !body.idempotencyKey) return NextResponse.json({ error: 'invalid_request' }, { status: 400 });
