...
export const faqPageContent = {
  ko: { 
    ...
    guide: '전체 안전 가이드 보기', 
    items: [
      [AlertOctagon, '백신이 파일을 위험하다고 표시하면 어떻게 해야 합니까?', '트레이너의 메모리 조작 방식 때문에 탐지될 수 있지만 모든 경고가 오진인 것은 아닙니다. 공식 배포처, 디지털 서명, 게시된 해시와 복수의 최신 검사 결과를 확인하고, 의심스러우면 실행하지 마십시오. 백신 예외 등록을 무조건 권장하지 않습니다.'],
      [HelpCircle, '지원되는 트레이너와 게임 버전이 다르면 어떻게 합니까?', '게임 업데이트로 인해 코드가 변경되었을 수 있습니다. 구버전 게임을 사용하거나 최신 트레이너가 지원될 때까지 기다리십시오.'],
      [Ban, '온라인에서 사용해도 안전합니까?', '아닙니다. 온라인·멀티플레이·경쟁·안티치트 환경에서는 사용하지 마십시오. 인터넷 연결을 끊거나 오프라인 모드를 선택하는 것만으로 제재 방지나 안전이 보장되지 않습니다. LocalPatcher는 안티치트 우회 방법을 제공하지 않습니다.'],
      [Lock, '선택한 트레이너 파일이 서버에 저장됩니까?', '변환 대상 파일은 브라우저에서 로컬로 처리되며 LocalPatcher 자체 데이터베이스에 업로드하거나 저장하지 않습니다. GA4가 접속·기기·이용 정보를 처리할 수 있고, 필요한 설정과 동의 준비가 완료된 경우 Google AdSense 광고가 표시될 수 있습니다. 자세한 내용은 개인정보처리방침을 확인하십시오.'],
    ]
  },
  en: { 
